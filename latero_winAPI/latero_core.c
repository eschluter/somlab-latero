...
int exchange_packet(latero_t* latero, latero_pkt_t* to_send, latero_pkt_t* response)
{
  ...
  if( socketIsReadable( latero->udp_socket, 500 ) ) {
#endif       
...
{
	latero->initialized = 0;
    if ( closesocket( latero->udp_socket ) != 0  ) 
    {
...
void latero_test_connection( latero_t* latero ) {
  latero_pkt_t response;
  latero_write( latero, &response );
...
{
	double refx, refy, a, b, x2, y2, x2py2, a2pb2;
	double angles[3];
    ...
    refx = FULL_WORKSPACE_WIDTH - DISC_DIAMETER/2 - ROOT_OFFSET_X;
    refy = FULL_WORKSPACE_HEIGHT - DISC_DIAMETER/2 - ROOT_OFFSET_Y - L2;
